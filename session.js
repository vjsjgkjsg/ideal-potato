...
const SESSION = (() => {
  ...
  async function sb(path, opts = {}) {
    ...
    const _tid = setTimeout(() => ctrl.abort(), 6000);
  ...
  // ── In-memory кеш (живёт пока страница открыта) ──
  ...
  async function getUser(forceRefresh = false) {
    ...
    if (!token) { _cachedUser = null; return null; }
    // Возвращаем кеш если свежий
    if (!forceRefresh && _cachedUser && (Date.now() - _cacheTime) < CACHE_TTL) {
      return _cachedUser;
    }
    try {
      const rows = await sb(
  ...
  // ── Установить пользователя напрямую (после createSession — без лишнего запроса) ──
