...
/* ══ ТАБЛИЦА ЛИДЕРОВ ══ */
const SB_URL_G  = 'https://ljtogliylpubvkckpwyv.supabase.co';
const SB_ANON_G = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxqdG9nbGl5bHB1YnZrY2twd3l2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzMxNzEzODQsImV4cCI6MjA4ODc0NzM4NH0.91EQ_EEfi8X09Vm0jmH12_35R6tf0BwtTGoX3K5lgfc';

function showLeaderboard() {
  const lb = document.getElementById('panel-leaderboard');
  const main = document.querySelector('.games-main') || document.querySelector('section.games');
  if (lb) lb.style.display = 'block';
  if (main) main.style.display = 'none';
  loadLeaderboard('all');
}

async function loadLeaderboard(game, btn) {
  // Update filter tabs
  if (btn) {
    document.querySelectorAll('#lb-game-filter .filter-tab').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
  }
  const el = document.getElementById('lb-list');
  if (!el) return;
  el.innerHTML = '<li style="text-align:center;padding:20px;color:var(--text3,#4a5f82)">Загрузка...</li>';
  try {
    const filter = game === 'all' ? '' : `&game_type=eq.${game}`;
    // Агрегируем по user_name — берём лучший результат каждого
    const r = await fetch(
      `${SB_URL_G}/rest/v1/game_records?select=user_name,score,game_type${filter}&order=score.desc&limit=100`,
      { headers: { apikey: SB_ANON_G, Authorization: 'Bearer ' + SB_ANON_G } }
    );
    const rows = (await r.json()) || [];
    // Группируем: лучший результат каждого игрока
    const map = {};
    rows.forEach(r => {
      if (!map[r.user_name] || r.score > map[r.user_name].score) map[r.user_name] = r;
    });
    const sorted = Object.values(map).sort((a,b) => b.score - a.score).slice(0, 20);
    if (!sorted.length) {
      el.innerHTML = '<li style="text-align:center;padding:20px;color:var(--text3,#4a5f82)">Пока никто не играл 🎮</li>';
      return;
    }
    el.innerHTML = sorted.map((p, i) => {
      ...
      return `<li class="lb-row">
        ...
        <div class="lb-badge">${p.game_type||'—'}</div>
        <div class="lb-score">${p.score} очк.</div>
      </li>`;
    }).join('');
  } catch(e) {
    el.innerHTML = '<li style="text-align:center;padding:20px;color:var(--text3)">Ошибка загрузки</li>';
  }
}
